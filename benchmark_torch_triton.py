...
import time
from triton_fp8 import gemm_split_k_kernel
import triton.ops
...
def benchmark_fp8_mm(m, n, k, dtype=torch.float16, qdtype=torch.float8_e4m3fn, num_iters=1000):
    ...
    x_fp8 = x.to(qdtype).contiguous()
    w_fp8 = w.to(qdtype).contiguous()
    ...
    print(f'{m}, {n}, {k}, {cos_sim_torch.item():.4f}, {cos_sim_triton.item():.4f}, {fp16_time:.4f}, {fp8_time:.4f}, {fp8_triton_time:.4f}, {fp16_time / fp8_time:.3f}, {fp16_time / fp8_triton_time:.3f}')
...
if __name__ == "__main__":
    ...
    print("M, N, K, Cosine similarity torch, Cosine similarity triton, FP16 time, FP8 torch time, FP8 triton time, Speedup torch, Speedup triton")
